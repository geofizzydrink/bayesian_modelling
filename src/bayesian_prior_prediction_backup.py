                ...
                if input_parameters != '':
                    if isinstance(input_parameters_JSON, list):
                        for model_run_params in input_parameters_JSON:
                            ...
                            # historic_control_arr = sp_stats.norm.rvs(loc = 1.715, scale = 0.05, size = 2001)
                            #===================================================
    ...
    if multiple_historic_datasets_flag:
        ...
        for historic_dataset in historic_data: 
            ...
            if len(historic_dataset)>0:
                historic_mu = np.linspace(np.min(historic_dataset), np.max(historic_dataset), num = len(historic_dataset))
            else:
        ...
        if len(historic_data)>0:
            historic_mu = np.linspace(np.min(historic_dataset), np.max(historic_dataset), num = len(historic_data))
        else:
            historic_mu = np.array([])
...
#===============================================================================
    ...
    #     print('min',min_historic_data)
    #     print('max',max_historic_data)
    #     print('len(borrowed_input_data)',len(borrowed_input_data))
    #     borrowed_mu = np.linspace(min_historic_data, max_historic_data, num = len(borrowed_input_data))
    # else:
    ...
    axes['prior'].plot(mu, borrowed_prior, label = 'Borrowed power_prior Distribution')
    ...
    for ind, datum in enumerate(borrowed_input_data):
        ...
        if ind%(len(borrowed_input_data)//5) == 0:
            axes['posterior_models'].plot(mu, normalized_posterior, label = f'Main Model after observing {ind} samples')
    ...
    top_bound = means = [compute_percentile(mu, prior_posterior_dict[i], 0.995) for i in range(num_x_samples)]
    bottom_bound = means = [compute_percentile(mu, prior_posterior_dict[i], 0.005) for i in range(num_x_samples)]
    means = [compute_percentile(mu, prior_posterior_dict[i], 0.5) for i in range(num_x_samples)]
    
    axes['regression'].axhline(y=compute_percentile(mu, prior_posterior_dict[len(borrowed_input_data)-1], 0.5), ls = '--', color = 'k', label = 'Prior Mean')
    ...
    axes['final_model'].plot(mu, prior_posterior_dict[len(borrowed_input_data)-1], label = 'Final Power Posterior Model')
    axes['final_model'].axvline(x = compute_percentile(mu, prior_posterior_dict[len(borrowed_input_data)-1], 0.5), ls = '--', color = 'r', label = 'Prior Model Mean')
    ...
    axes['final_model'].set_xlim(compute_percentile(mu, prior_posterior_dict[len(borrowed_input_data)-1], 0.00005),
                                 compute_percentile(mu, prior_posterior_dict[len(borrowed_input_data)-1], 0.99995))
    ...
    return prior_posterior_dict[len(borrowed_input_data)-1] #posterior_mean, posterior_std
